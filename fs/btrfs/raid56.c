...
// add by karla
	...
	rbio->inode = inode->i_ino;
	printk(KERN_INFO "From alloc_rbio, inode id is %lu", rbio->inode);
#endif
// add by karla
...
// add by karla 
#ifdef CONFIG_BIO_WITH_INODE_ID
	bio->i_ino = rbio->inode->i_ino;
	//bio->_pid = current->pid;
	printk(KERN_INFO "From finish_rmw, inode id is %lu", bio->i_ino);
#endif
// add by karla 
...
static int raid56_rmw_stripe(struct btrfs_raid_bio *rbio)
{
	...
	bio->i_ino = rbio->inode->i_ino;
	//bio->_pid = current->pid;
	printk(KERN_INFO "From raid56_rmw_stripe, inode id is %lu", bio->i_ino);
#endif
	// add by karla 
...
// add by karla
	...
	bio->i_ino = rbio->inode->i_ino;
	//bio->_pid = current->pid;
	printk(KERN_INFO "From __raid56_parity_recover, inode id is %lu", bio->i_ino);
#endif
	// add by karla
...
// add by karla 
	...
	bio->i_ino = rbio->inode->i_ino;
	//bio->_pid = current->pid;
	printk(KERN_INFO "From finish_parity_scrub, inode id is %lu", bio->i_ino);
#endif
// add by karla 
...
// add by karla
	...
	bio->i_ino = rbio->inode->i_ino;
	//bio->_pid = current->pid;
	printk(KERN_INFO "From raid56_parity_scrub_stripe, inode id is %lu", bio->i_ino);
#endif
// add by karla
