...
// add by karla 
	...
	bio->i_ino = BTRFS_I(inode)->vfs_inode.i_ino;
	//bio->_pid = current->pid;
	printk(KERN_INFO "From btrfs_submit_metadata_bio, inode id is %lu", bio->i_ino);
#endif
// add by karla 
	...
	if (btrfs_op(bio) != BTRFS_MAP_WRITE) {
		 ...
		 */
		ret = btrfs_wq_submit_bio(inode, bio, mirror_num, 0, 0,
