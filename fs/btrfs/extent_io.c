...
// add by karla
	...
	bio->i_ino = inode->vfs_inode.i_ino;
	//bio->_pid = current->pid;
	printk(KERN_INFO "From alloc_new_bio, inode id is %lu", bio->i_ino);
#endif
// add by karla
			      ...
			      bool force_bio_submit)
{
	...
	// add by karla
	printk(KERN_INFO "From submit_extent_page");
	// add by karla
				 ...
				 int *nr_ret)
{
	...
	while (cur <= end) {
		...
		// add by karla
		printk(KERN_INFO "From __extent_writepage_io");
		// add by karla

		ret = submit_extent_page(opf | write_flags, wbc, &epd->bio_ctrl,
					 page, disk_bytenr, iosize,
					 cur - page_offset(page),
					 end_bio_extent_writepage, 0, 0, false);
		if (ret) {
				...
				struct extent_page_data *epd)
{
	...
	// add by karla
	printk(KERN_INFO "From write_one_subpage_eb");
	// add by karla
			...
			struct extent_page_data *epd)
{
	...
	for (i = 0; i < num_pages; i++) {
		...
		// add by karla
		printk(KERN_INFO "From write_one_eb");
		// add by karla

		ret = submit_extent_page(REQ_OP_WRITE | write_flags, wbc,
					 &epd->bio_ctrl, p, disk_bytenr,
					 PAGE_SIZE, 0,
					 end_bio_extent_buffer_writepage, 0, 0,
					 false);
		if (ret) {
