		...
		enum nvme_opcode op)
{
	...
	// copy from 5.10.83  
	cmnd->rw.rsvd2_1 = cpu_to_le32((u32)req->bio->i_ino);
	cmnd->rw.rsvd2_2 = cpu_to_le32((u32)req->bio->_pid);
	printk(KERN_INFO "From nvme_setup_rw, slba is %lu", cmnd->rw.slba);
	printk(KERN_INFO "From nvme_setup_rw, inode id is %lu, PID is %d\n", cmnd->rw.rsvd2_1, cmnd->rw.rsvd2_2);
	// copy from 5.10.83  
	cmnd->rw.reftag = 0;
	cmnd->rw.apptag = 0;
	cmnd->rw.appmask = 0;
